/**
 * Citizen Job Board - Bandi di Lavoro
 * Esplora posizioni aperte e candidati
 */
...
const CitizenJobsPage = () => {
  const { token } = useAuth();
  const [tab, setTab] = useState('browse'); // browse, apply, my-applications
  const [jobs, setJobs] = useState([]);
  const [myApps, setMyApps] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedJob, setSelectedJob] = useState(null);
  const [form, setForm] = useState({ motivation: '', experience: '', availability: '', additional_info: '' });
  ...
  const selectJob = (job) => {
    setSelectedJob(job);
    setTab('apply');
    setForm({ motivation: '', experience: '', availability: '', additional_info: '' });
  };
  ...
  const handleApply = async () => {
    ...
    setSubmitting(true);
    try {
      await axios.post(`${API}/api/jobs/${selectedJob.id}/apply`, form, { headers });
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex flex-col items-end gap-1 ml-3 flex-shrink-0">
                  {job.user_applied ? (
                    ...
                    <button onClick={() => selectJob(job)} className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-[#adff2f]/10 border border-[#adff2f]/20 text-[#adff2f] text-xs font-bold hover:bg-[#adff2f]/20 transition-colors" data-testid={`apply-btn-${job.id}`}>
                      <Send size={12} />
                      CANDIDATI
                    </button>
                  )}
        ...
        <div className="space-y-3">
          ...
          <div className="bg-[#0a0e12] border border-[#adff2f]/15 rounded-lg p-4 space-y-3" data-testid="apply-form">
            <div>
              ...
              <p className="text-[0.625rem] text-[#7f9aa3]">{selectedJob.department} - {selectedJob.location}</p>
            </div>
            <div>
              <label className="text-[0.625rem] text-[#7f9aa3] uppercase tracking-wider">Motivazione *</label>
              <textarea value={form.motivation} onChange={e => setForm({...form, motivation: e.target.value})} className="w-full mt-1 px-3 py-2 bg-[#060a0d] border border-[#1b2a30] rounded text-sm text-white h-24 resize-none" placeholder="Perche' vuoi questo lavoro?" data-testid="apply-motivation" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[0.625rem] text-[#7f9aa3] uppercase tracking-wider">Esperienza</label>
                <textarea value={form.experience} onChange={e => setForm({...form, experience: e.target.value})} className="w-full mt-1 px-3 py-2 bg-[#060a0d] border border-[#1b2a30] rounded text-sm text-white h-16 resize-none" placeholder="Esperienza precedente..." />
              </div>
              <div>
                <label className="text-[0.625rem] text-[#7f9aa3] uppercase tracking-wider">Disponibilita'</label>
                <input value={form.availability} onChange={e => setForm({...form, availability: e.target.value})} className="w-full mt-1 px-3 py-2 bg-[#060a0d] border border-[#1b2a30] rounded text-sm text-white" placeholder="Es: Lun-Ven, 18-23" />
                <label className="text-[0.625rem] text-[#7f9aa3] uppercase tracking-wider mt-2 block">Note Aggiuntive</label>
                <input value={form.additional_info} onChange={e => setForm({...form, additional_info: e.target.value})} className="w-full mt-1 px-3 py-2 bg-[#060a0d] border border-[#1b2a30] rounded text-sm text-white" placeholder="Altro da aggiungere..." />
              </div>
            </div>
            <button onClick={handleApply} disabled={submitting} className="flex items-center gap-2 px-4 py-2 bg-[#adff2f] text-[#060a0d] rounded font-bold text-xs hover:bg-[#c5ff5e] transition-colors disabled:opacity-50" data-testid="submit-apply-btn">
              ...
              <div key={app.id} className="bg-[#0a0e12] border border-[#1b2a30] rounded-lg p-3" data-testid={`my-app-${app.id}`}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[0.625rem] text-[#7f9aa3]">{app.job_department} - Inviata il {app.created_at?.slice(0,10)}</p>
                  </div>
                ...
                {app.reviewer_notes && (
                  <div className="mt-2 pl-0 text-xs bg-[#adff2f]/5 border border-[#adff2f]/10 rounded p-2">
